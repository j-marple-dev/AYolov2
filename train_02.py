...
from val2 import ModelLoader, export_model
...
def get_parser() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model",
        type=str,
        default=os.path.join("res", "configs", "model", "yolov5xx.yaml"),
        help="Model "
        + colorstr("config")
        + " or "
        + colorstr("weight")
...
if __name__ == "__main__":
    ...
    # AIGC reproduce
    ckpt_path = os.path.join(train_builder.log_dir, "weights", "best.pt")
    #model = torch.load(ckpt_path)["model"]
    #model = model.half().fuse().eval().float()
    #torch.save(model.cpu(), './best.pt')
    import argparse
    new_args = argparse.Namespace()
    new_args.weights = ckpt_path
    new_args.model_cfg = ""
    new_args.half = False

    torch.set_grad_enabled(False)
    model_loader = ModelLoader(new_args, torch.device("cuda:0"))
    model_loader.start()
    model_loader.join()
    export_model(model_loader, new_args)
